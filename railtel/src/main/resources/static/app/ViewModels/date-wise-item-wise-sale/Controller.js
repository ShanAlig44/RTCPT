var app = angular.module('dateWiseItemWiseSaleApp', []);
app.controller('dateWiseItemWiseSaleCtrl', function(dataService,$scope,$location) {
	debugger;
	...
	dataService.getSaleDateWiseItemWise(data).then(
			function(response) {
				debugger
				if(response.data.data.datewiseItemwiseSale==null){
				...
				if (response.status == 200) {		
					...
					$scope.datewiseItemwiseSale 	=   response.data.data.datewiseItemwiseSale
	...
	$scope.showDateWiseData = function() {
		...
		var url='date-wise-item-wise-sale#?fromDate='+$scope.fromDate+'&toDate='+$scope.toDate;
