var app = angular.module('dateWiseItemWisePurchaseApp', []);
app.controller('dateWiseItemWisePurchaseCtrl', function(dataService,$scope,$location) {
	debugger;
	...
	dataService.getPurchaseDateWiseItemWise(data).then(
			function(response) {
				debugger
				if(response.data.data.datewiseItemwisePurchase==null){
				...
				if (response.status == 200) {		
					...
					$scope.datewiseItemwisePurchase =   response.data.data.datewiseItemwisePurchase
	...
	$scope.showDateWiseData = function() {
		...
		var url='date-wise-item-wise-purchase#?fromDate='+$scope.fromDate+'&toDate='+$scope.toDate;
