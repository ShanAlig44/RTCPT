var app = angular.module('billsPayableApp', []);
app.controller('billsPayableCtrl', function(dataService,$scope,$location) {
	debugger;
	...
	dataService.getBillPayable(data).then(
			function(response) {
				debugger
				if(response.data.data.outstanding.payable==null){
				...
				if (response.status == 200) {		
					...
					$scope.billsPayable 			=   response.data.data.outstanding.payable
	...
	$scope.dateWiseData = function() {
		...
		var url='bills-payable#?fromDate='+$scope.fromDate+'&toDate='+$scope.toDate;
	...
	$scope.showTotal = function() {
		debugger;
		var totalPendingAmpunt = null;
		for(var i =0; i<$scope.billsPayable.length; i++){
			totalPendingAmpunt =+$scope.billsPayable[i].billClosing;
			
		}
		alert(totalPendingAmpunt)
	}
