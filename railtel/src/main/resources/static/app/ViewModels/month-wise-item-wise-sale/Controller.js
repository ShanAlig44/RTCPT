var app = angular.module('monthWiseItemWiseSaleApp', []);
app.controller('monthWiseItemWiseSaleCtrl', function(dataService,$scope,$location) {
	debugger;
	...
	dataService.getSaleMonthWiseItemWise(data).then(
			function(response) {
				debugger
				if(response.data.data.monthwiseItemwiseSale==null){
				...
				if (response.status == 200) {		
					...
					$scope.monthwiseItemwiseSale=   response.data.data.monthwiseItemwiseSale
	...
	$scope.showDateWiseData = function() {
		...
		var url='month-wise-item-wise-sale#?fromDate='+$scope.fromDate+'&toDate='+$scope.toDate;
